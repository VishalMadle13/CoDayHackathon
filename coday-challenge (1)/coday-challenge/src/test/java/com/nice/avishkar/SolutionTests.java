...
public class SolutionTests {
	...
	@BeforeClass
	public static void initAndExecute() throws FileNotFoundException {
		...
		resultData.getResultData().stream().forEach(constituencyResult -> {
			...
			switch (constituencyResult.getConstituencyName()){
				...
				case "Niphad":
					...
					Assert.assertTrue(constituencyResult.getCandidateList().get(0).getCandidateName().equals("ChidambarPanicker"));
					Assert.assertTrue(constituencyResult.getCandidateList().get(0).getVotes()==255);
					break;
				case "Hingna":
